...
public class ExcelTool { 
	...
	public static FileInputStream exportExcel(String template,
		...
		InputStream is = null;
		try {
//			File file = new File("src/main/java/com/suntak/eightdisciplines/ExcelUtil/template/" + template);
//			is = new FileInputStream(file);

			// 上述方式在IDE中没有问题，但是打成jar包上线后，文件目录就不是这样了，需要通过流的方式获取数据

			ClassPathResource resource = new ClassPathResource("static/excel/" + template);
	...
	public static WritableCell getValueByExp1(ExcelCells cells,
		...
		for (String exp : exps) {
			...
			if (!old_c.isEmpty() && old_c.toString() != "") {
				while (old_c.indexOf(exp) != -1) {
	...
	public static synchronized String customExportExcel(int beginRow,
			int beginColumn, Map heads, List result) {
		return null;
	}
